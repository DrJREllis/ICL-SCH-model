...
def setupSD(params):
    ...
    if params['adherenceSetting'] == "betabinomial": 
        ...
        adherenceFactors = np.zeros(params['N'])
        adherenceFactors[ageCoverage<1] = np.random.beta(a=ageCoverage[ageCoverage<1]* (1 - params['rhoAdherence'])/params['rhoAdherence'],
                                            b=(1 - ageCoverage[ageCoverage<1])*(1 - params['rhoAdherence'])/params['rhoAdherence'], size=np.sum(ageCoverage<1)) 
        adherenceFactors[ageCoverage==1] = 1
        adherenceFactors[np.isnan(ageCoverage)] = 0
...
def getAttendance(adherenceFactors, coverage, treatmentAgeGroupIndices):
    c = coverage[treatmentAgeGroupIndices]
    p = adherenceFactors^((1-c)/c) 
    attendance = np.random(len(p)) < p # get a random number for each host and they attend if that's lower than their probability.
...
def doFreeLive(params, SD, dt):
    ...
    eggOutputPerHost = params['lambda'] * productivefemaleworms * np.exp(-SD['worms']['total'] * params['gamma']) # Exponential to total worms or productivefemaleworms?
...
def doDeath(params, SD, t):
    ...
    if len(theDead) != 0:
        ...
        SD['adherenceFactors'][theDead] = np.where(params['coverage'][0]==0, np.nan, params['coverage'][0])
    ...
    if params['adherenceSetting']=="betabinomial":
        ...
        if sum(params['coverage'][SD['treatmentAgeGroupIndices']] != params['coverage'][newTreatmentAgeGroup])>0:
            ...
            for i in agedIndices[0]:
                ageCoverage = np.where(params['coverage'][newTreatmentAgeGroup[i]]==0, np.nan, params['coverage'][newTreatmentAgeGroup[i]]/(1 - params['propNeverCompliers']))
                if ageCoverage<1: 
                    SD['adherenceFactors'][i] = np.random.beta(
                        ageCoverage* (1 - params['rhoAdherence'])/params['rhoAdherence'],
                        b=(1 - ageCoverage)*(1 - params['rhoAdherence'])/params['rhoAdherence'], 
                        size=1)#sum(params['coverage'][SD['treatmentAgeGroupIndices']] != params['coverage'][newTreatmentAgeGroup]))
                if np.isnan(ageCoverage): SD['adherenceFactors'][i] = 0
                if ageCoverage==1: SD['adherenceFactors'][i] = 1
...
def getSetOfEggCounts(total, female, params, Unfertilized=False):
    ...
    if params['reproFuncName'] == 'epgMonog':
        ...
        meanCount = eggProducers * params['lambda'] * params['z'] ** total # Female or total?

    if params['reproFuncName'] == 'epgFertility':
...
def getPrevalence(hostData, params, numReps, nSamples=2, Unfertilized=False, villageSampleSize=100): # changed village sample size from 100 to 50 as a test
...
def getIndividualResults(hostData,params,nSamples=2,Unfertilized=False, villageSampleSize=100):
    ...
    worm_results = np.zeros(np.shape(egg_results))
    lifetime_results = np.zeros(np.shape(egg_results))
    yearsInfected_results = np.zeros(np.shape(egg_results))
    host_ages = np.zeros(np.shape(egg_results))
    # attendance = np.zeros((np.size(hostData[0]['attendanceRecord'],0),np.size(hostData[0]['attendanceRecord'],1),len(hostData)))
    # ageAtChemo = np.zeros((params['N'],params['numReps'],params['nRounds'] ))
    for i in range(len(hostData)):
        ...
        # ageAtChemo[:,i,:] = hostData[i]['ageAtChemo'].transpose()
        # attendance[:,:,i] = hostData[i]['attendanceRecord']


    individualResults = dict(egg_results=egg_results.tolist(), ages=host_ages.tolist(),worm_results=worm_results.tolist(),
                         lifetime_results=lifetime_results.tolist(), yearsInfected_results=yearsInfected_results.tolist() )
    
    return individualResults
    

def getTargetThresholdProbabilities(individualResults,params):

    egg_results = np.array(individualResults['egg_results'])
    worm_results = np.array(individualResults['worm_results'])
    numReps = np.size(egg_results,axis=1)
    
    eggCount_0 = np.sum(np.sum(egg_results,axis=2)==0, axis=1) / numReps
    eggCount_1 = np.sum(np.sum(egg_results>0,axis=2)/params['N']<=0.01, axis=1) / numReps
    eggCount_2 = np.sum(np.sum(egg_results>0,axis=2)/params['N']<=0.02, axis=1) / numReps
    # heavyIntensity_1 = np.sum(sac_heavy_results<0.01, axis=1) / numReps
    # who_goal_5 = np.sum(sac_heavy_results<0.05, axis=1) / numReps

    # chemoStart = list(params['outTimings']).index(params['chemoTimings'][0]) - 1
    # heavyIntensity_1 =  np.sum(sac_heavy_results[:,egg_results[chemoStart,:]>0]<0.01, axis=1) / np.sum(egg_results[chemoStart,:]>0)

    heavyIntensity_1 =  np.sum(np.sum(egg_results>params['burdenThreshold'],axis=2)<=0.01, axis=1) / numReps

    # egg_results = np.sum(egg_results, axis=1) / numReps

    wormCount_0 = np.sum(np.sum(worm_results,axis=2)==0,axis=1)/numReps
    wormCount_1 = np.sum(np.sum(worm_results>0,axis=2)/params['N']<=0.01,axis=1)/numReps
    wormCount_2 = np.sum(np.sum(worm_results>0,axis=2)/params['N']<=0.02,axis=1)/numReps
    
    df = pd.DataFrame({'Time': params['outTimings'],
                        'eggCount_0':eggCount_0,
                        'eggCount_1':eggCount_1,
                        'eggCount_2':eggCount_2,
                        'heavyIntensity_1': heavyIntensity_1,
                        'wormCount_0': wormCount_0,
                        'wormCount_1':wormCount_1,
                        'wormCount_2':wormCount_2
                    })
